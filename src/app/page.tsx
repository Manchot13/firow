...
import { handleState, clockSwitch, toDoSwitch, breatheModalSwitch, eyeModalSwitch, fontType, pomodoroModalSwitch } from '@/globalStateAtoms/atoms';
...
export default function Home() {
  ...
  return (
    <ClientOnly>
      <div className={`relative flex min-h-screen bg-snow-100 font-[family-name:var(--font-geist-sans)] tracking-[1em] ${isFontType === "JaPixel" ? styles.DotGothic16 : isFontType === "EnPixel" ? styles.cofoSansPixel : "font-mono"}`}>
        <main className="h-full min-h-screen w-full relative">
          ...
          <div className="absolute right-6 top-4 text-2xl flex md:flex-col-reverse md:items-end md:right-2">
            ...
            <div className='flex justify-center tracking-wider gap-[10%] mr-4 md:hidden'>
              <div className=" cursor-pointer rounded-md flex justify-center hover:bg-snow-200 w-fit h-full md:pr-0" onClick={() => (setEyeOn(!isEyeOn))}>
                <PageFunction title="Eye" icon={isEyeOn ? <LiaEyeSolid /> : <RxEyeClosed />} />
              </div>
